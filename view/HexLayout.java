...
public class HexLayout implements LayoutManager {
    ...
    private Integer calculateRowCount(Container parent) {
        ...
        // startEvenRow is true then rowCount = 1, otherwise rowCount = 0
        int rowCount = (this.startEvenRow) ? 1 : 0;
        int cells = parent.getComponents().length;
        
        while (cells > 0) {
            cells -= this.columns - (rowCount % 2) + 1;
            rowCount++;
        }
    ...
    public void layoutContainer(Container parent) {
        ...
        for (Component component : parent.getComponents()) {
            ...
            currentColumn++;
            if ((evenRow && currentColumn == this.columns) || (!evenRow && currentColumn == this.columns)) {
                currentColumn = 0;
                currentRow++;
                iteration++;
            }
